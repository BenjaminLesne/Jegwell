...
export function handleAddToBasket(button: HTMLButtonElement) {
  ...
  const product_option =
    button.getAttribute("data-product-option") ?? "default";
  const product_quantity_stringified = button.getAttribute(
  ...
  if (product_id == null) {
    ...
    console.error(
      "product_id == null || product_quantity_stringified == null - this is true"
    );
    ...
    if (currentProductsAddedJson != null) {
      ...
      const productIndex = currentProductsAdded.findIndex(
        (product: Product) =>
          product?.id === product_id && product?.option === product_option
      );
      ...
      newProductsAddedJson = JSON.stringify([
        { id: product_id, option: product_option, quantity: quantity },
      ]);
    }
...
export function removeProductFromBasket(
  ...
  if (currentProductsAddedJson) {
    ...
    console.error(
      "removeProductFromBasket() -> cookie productsToBasket is empty. We can't remove a product that doesn't exist in the basket"
    );
  }
}
...
export function openQuantityModal(button: HTMLButtonElement) {
  ...
  const quantityModal: HTMLDialogElement | null =
    document.querySelector("#quantityModal");
  const quantityValueModal = quantityModal?.querySelector(
    ".quantity-setter__value"
  );
  ...
  if (condition) {
    ...
    console.error(
      "button doesn't have a filled 'data-product-id' attribute or 'data-quantity' > 0"
    );
...
export function handleQuantity(isAddition: boolean, button: HTMLButtonElement) {
  ...
  const modalQuantityValueElement = modal?.querySelector(
    ".quantity-setter__value"
  );
  const currentQuantity = modal?.getAttribute("data-quantity")
    ? modal.getAttribute("data-quantity")
    : modalQuantityValueElement?.textContent;
  let newQuantity;

  if (
    currentQuantity != null &&
    modal != null &&
    modalQuantityValueElement != null
  ) {
...
export function handleBasketQuantityConfirmation(button: HTMLElement) {
  ...
  if (
    ...
    const message =
      "handleBasketConfirmation -> modal or productId or quantity or productOption is undefined/null";
...
export function handleSinglePageQuantityConfirmation(button: HTMLElement) {
  ...
  const quantityValueElement = quantitySettingButton?.querySelector(
    ".setting__value-span"
  );
  ...
  if (
    quantityValueElement?.textContent != null &&
    quantity != null &&
    modal != null &&
    ...
    console.error(
      "quantityValueElement?.textContent != null && quantity != null && modal != null - this condition is false"
    );
...
export function openOptionsModal(button: HTMLButtonElement) {
  const productId = button.getAttribute("data-product-id");
  const productOption = button.getAttribute("data-product-option");
  const optionsModal: HTMLDialogElement | null = document.querySelector(
    `.optionsModal[data-product-id="${productId}"]`
  );

  const condition =
    productId != null && optionsModal != null && productOption != null;
  ...
  if (condition) {
    ...
    const message =
      "button doesn't have a filled 'data-product-id' attribute or 'data-option'";
    console.error(message);
    alert("Aïe, une erreur est survenue :(");
    throw new Error(message);
  }
...
export function showSelectedOption(
  productId: string,
  productOption: string,
  optionsModal: HTMLDialogElement
) {
  const currentOption = optionsModal?.querySelector(
    `.product-option-wrapper.selected`
  );
  const newOption = optionsModal?.querySelector(
    `.product-option-wrapper[data-product-option='${productOption}']`
  );
  ...
  if (newOption) {
    ...
    const message =
      "currentOption, newOption or optionsModal is undefined/null";
    console.error(message);
    const variables = {
...
export function handleOptionConfirm(optionsModal: HTMLDialogElement | null) {
  const optionElementSelected = optionsModal?.querySelector(
    ".product-option-wrapper.selected"
  );
  const optionSelected = optionElementSelected?.getAttribute(
    "data-product-option"
  );
  ...
  if (condition) {
    ...
    const message =
      "optionSelected or previousOption or productId is undefined";
    console.table([optionSelected, previousOption, productId]);
    throw new Error(message);
  }
}

export function handleSingleProductOptionConfirm(
  optionsModal: HTMLDialogElement | null
) {
  const optionElementSelected = optionsModal?.querySelector(
    ".product-option-wrapper.selected"
  );
  ...
  const valueContainer = settingOptionButton?.querySelector(
    ".setting__value-span"
  );
  const mediaScroller = document.querySelector(".media-scroller");
  const targetMediaElement = document.querySelector(
    `.media-element[data-product-option="${optionSelected}"]`
  );
...
function updateBasket(
  ...
  if (currentProductsAddedJson) {
    ...
    switch (true) {
      ...
      case key === "option":
        {
          const targetedProductIndex = currentProductsAdded.findIndex(
            (product: Product) => {
              const result =
                product?.id === productId && product?.option === value;
          ...
          if (targetedProductIndex > -1) {
            ...
            if (currentProductsAdded[productIndex] != null) {
              ...
              addToCookies(
                "productsToBasket",
                JSON.stringify(currentProductsAdded),
                365
              );
    ...
    console.error(
      "updateQuantity() -> cookie productsToBasket is empty. We can't update a product that doesn't exist in the basket"
    );
  }
}
...
export function handleFormSubmit(event: MouseEvent) {
  ...
  if (form != null) {
    ...
    const deliveryChosen = document.querySelector(
      ".form__input--radio:checked"
    );
    const deliveryChosenInvalid = document
      .querySelector(".form__input--radio")
      ?.parentElement?.closest(".form__fieldset--radio.invalid");
  ...
  function handleInputValidation(
    ...
    const isRegexFollowed = additionalCondition;

    const inputLabelWrapper = input?.parentElement?.closest(
...
function addToCookies(key: string, value: string, days: number) {
  const HOME_PATH =
    document
      .querySelector(".primary-header-wrapper")
