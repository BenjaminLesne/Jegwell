...
const addToBasketButtons: NodeListOf<HTMLButtonElement> =
  document.querySelectorAll(".product__call-to-action-wrapper");
const modalCloseButtons = document.querySelectorAll(".close-button--modal");
const quantitySetterButtons: NodeListOf<HTMLButtonElement> =
  document.querySelectorAll(".quantity-setter__button");
const quantityButtons: NodeListOf<HTMLButtonElement> =
  document.querySelectorAll(".setting--quantity");

const optionsButtons: NodeListOf<HTMLButtonElement> =
  document.querySelectorAll(".setting--option");
const productOptionWrapperAll: NodeListOf<HTMLButtonElement> =
  document.querySelectorAll(".product-option-wrapper");
...
productOptionWrapperAll.forEach((button) => {
  ...
  if (productOption != null && productId != null && modal != null) {
    button.addEventListener("click", () =>
      showSelectedOption(productId, productOption, modal)
    );
  }
});
